...
public class ProfessionManager {
    ...
    public void initialize() {
        if (initialized) {
            XeenaaVillagerManager.LOGGER.warn("ProfessionManager already initialized");
            return;
        }

        XeenaaVillagerManager.LOGGER.info("Initializing ProfessionManager - collecting all professions");
        collectProfessions();
        identifyVanillaProfessions();
        initialized = true;

        XeenaaVillagerManager.LOGGER.info("ProfessionManager initialized with {} total professions ({} vanilla, {} modded)",
    ...
    private void collectProfessions() {
        ...
        for (VillagerProfession profession : Registries.VILLAGER_PROFESSION) {
            ...
            if (id != null) {
                ...
                XeenaaVillagerManager.LOGGER.debug("Found profession: {} ({})", id, profession);
            }
        }

        XeenaaVillagerManager.LOGGER.info("Collected {} professions from registry", professions.size());
    }
    ...
    private void identifyVanillaProfessions() {
        ...
        XeenaaVillagerManager.LOGGER.info("Identified {} vanilla professions", vanillaProfessions.size());
    }
    ...
    public void logAllProfessions() {
        ...
        XeenaaVillagerManager.LOGGER.info("=== PROFESSION REGISTRY DETAILS ===");
        ...
        for (ProfessionData data : allData) {
            ...
            XeenaaVillagerManager.LOGGER.info("[{}] {} ({}) - Workstation: {} - Display: '{}'",
                type, data.getId(), data.getName(), workstation, data.getTranslatedName().getString());
        }

        XeenaaVillagerManager.LOGGER.info("=== END PROFESSION REGISTRY ===");
    }
     ...
     */
    public void refresh() {
        XeenaaVillagerManager.LOGGER.info("Refreshing ProfessionManager");
        initialized = false;
        initialize();
    }
